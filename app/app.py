...
def pick_path(filename: str) -> Path:
    ...
    raise FileNotFoundError(
        f"Não encontrei {filename}. Procurei em: " + " | ".join(str(x) for x in CANDIDATOS_DATA)
    )
...
try:
    PESOS = load_json(pick_path("pesos_velox.json"))
    INDICADORES = load_json(pick_path("empresa_indicadores_velox.json"))
